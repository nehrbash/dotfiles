...
Item {
    ...
    onOccupiedChanged: {
        ...
        if (pills.length > count) {
            const removed = pills.splice(count, pills.length - count);
            // Delay destruction to avoid use-after-free during Repeater incubation
            Qt.callLater(() => removed.forEach(p => p.destroy()));
        }
    }
