...
- [[#ui-look][UI Look]]
  ...
  - [[#tab-bar][Tab bar]]
  - [[#general][General]]
  - [[#terminal-mods][Terminal mods]]
  - [[#ligatures][Ligatures]]
- [[#general-settings][General Settings]]
  - [[#setting-emacs-defaults][Setting Emacs Defaults]]
  - [[#mouse][Mouse]]
...
*** compile buffer
    ...
    When output is designed for shell you get a lot of coloring text and it looks bad. Instead of fighting tools that do this we can just apply the ANSI coloring.
...
** Mouse
   ...
   I wanted just a really simple right click menu.
...
** Auto Save
   ...
   I have format on save hooks so keep long interval.
...
Keypad prefix translations (SPC in normal mode enters keypad):
...
The leader map (~SPC SPC~) bypasses keypad translation entirely — bindings are direct with no ~C-c~ prepending.

**** Leader Map (~SPC~)

~SPC~ is bound directly in normal and motion state to ~my-leader-map~, bypassing keypad translation entirely.

 | Key     | Command                     | Description           |
 |---------+-----------------------------+-----------------------|
 | ~SPC SPC~ | ~consult-buffer~          | Buffer switcher       |
 | ~SPC /~ | ~consult-ripgrep~           | Ripgrep               |
 | ~SPC j~ | ~meow-comment~              | Comment               |
 | ~SPC a~ | ~agent-shell~               | AI chat               |
 | ~SPC i~ | ~consult-imenu~             | Imenu                 |
 | ~SPC y~ | ~consult-yank-pop~          | Yank ring             |
...
***** ~SPC s~ — Search
 ...
 | Key     | Command                   | Description           |
 |---------+---------------------------+-----------------------|
 | ~s s~   | ~consult-line~            | Line search           |
 | ~s S~   | ~consult-line-multi~      | Multi-buffer search   |
 | ~s r~   | ~consult-ripgrep~         | Ripgrep               |
 | ~s g~   | ~consult-grep~            | Grep                  |
 | ~s G~   | ~consult-git-grep~        | Git grep              |
 | ~s d~   | ~consult-find~            | Find file by name     |
 | ~s c~   | ~consult-locate~          | Locate                |
 | ~s k~   | ~consult-keep-lines~      | Keep matching lines   |
 | ~s u~   | ~consult-focus-lines~     | Focus matching lines  |
 | ~s e~   | ~consult-isearch-history~ | Isearch history       |
 | ~s w~   | ~google-this~             | Web search            |
 | ~s D~   | ~define-word-at-point~    | Define word           |
...
***** ~SPC f~ — File
 ...
 | ~f .~   | ~find-file-at-point~  | Open file at point |
 | ~f r~   | ~consult-recent-file~ | Recent files       |
 | ~f d~   | ~consult-dir~         | Directory jump     |
 | ~f s~   | ~yas-insert-snippet~  | Insert snippet     |

***** ~SPC b~ — Buffer / Bookmark

 | Key     | Command                        | Description         |
 |---------+--------------------------------+---------------------|
 | ~b b~   | ~consult-buffer~               | Buffer list         |
 | ~b o~   | ~consult-buffer-other-window~  | Other window        |
 | ~b O~   | ~consult-buffer-other-frame~   | Other frame         |
 | ~b t~   | ~consult-buffer-other-tab~     | Other tab           |
 | ~b m~   | ~consult-bookmark~             | Bookmarks           |
 | ~b i~   | ~ibuffer~                      | Ibuffer             |
...
***** ~SPC g~ — Goto / Navigation
 ...
 | Key     | Command                      | Description         |
 |---------+------------------------------+---------------------|
 | ~g g~   | ~consult-goto-line~          | Goto line           |
 | ~g o~   | ~consult-outline~            | Outline             |
 | ~g i~   | ~consult-imenu~              | Imenu               |
 | ~g I~   | ~consult-imenu-multi~        | Imenu multi-buffer  |
 | ~g m~   | ~consult-mark~               | Marks               |
 | ~g M~   | ~consult-global-mark~        | Global marks        |
 | ~g e~   | ~consult-compile-error~      | Compile errors      |
 | ~g f~   | ~consult-flymake~            | Flymake errors      |
 | ~g d~   | ~consult-xref-stack-backward~| Xref back           |

***** ~SPC p~ — Project

 | Key     | Command                            | Description         |
 |---------+------------------------------------+---------------------|
 | ~p p~   | ~project-prefix-map~               | Project commands    |
 | ~p d~   | ~flymake-show-project-diagnostics~ | Project diagnostics |
 | ~p m~   | ~sn/project-menu~                  | Project menu        |
...
***** ~SPC n~ — Notes (org-roam)
 ...
 | ~n i~   | ~org-roam-node-insert~               | Insert node link  |
 | ~n n~   | ~org-roam-buffer-display-dedicated~  | Roam buffer       |
 | ~n g~   | ~org-roam-graph~                     | Graph             |
 | ~n d~   | ~org-roam-dailies-goto-date~         | Daily by date     |
 | ~n t~   | ~org-roam-dailies-goto-today~        | Today             |
 | ~n y~   | ~org-roam-dailies-goto-yesterday~    | Yesterday         |
 | ~n r~   | ~org-roam-dailies-goto-tomorrow~     | Tomorrow          |
 | ~n c~   | ~org-roam-dailies-capture-today~     | Capture today     |
 | ~n C~   | ~org-roam-dailies-capture-tomorrow~  | Capture tomorrow  |
...
***** ~SPC o~ — Org
 ...
 | ~o j~   | ~org-clock-goto~  | Clock goto     |
 | ~o i~   | ~consult-clock-in~| Clock in       |
 | ~o o~   | ~org-clock-out~   | Clock out      |
 | ~o l~   | ~org-clock-in-last~ | Clock in last|
...
***** ~SPC w~ — Window
 ...
 | ~w o~   | ~switch-window~                                      | Switch window     |
 | ~w 2~   | ~split-window-func-with-other-buffer-vertically~     | Split vertical    |
 | ~w 3~   | ~split-window-func-with-other-buffer-horizontally~   | Split horizontal  |
 | ~w 1~   | ~sn-toggle-delete-other-windows~                     | Toggle one window |
 | ~w \vert~| ~split-window-horizontally-instead~                 | Swap to horiz     |
 | ~w _~   | ~split-window-vertically-instead~                    | Swap to vert      |
 | ~w d~   | ~sn-toggle-current-window-dedication~                | Dedicate window   |
 | ~w s~   | ~sn-split-window~                                    | Smart split       |

***** ~SPC r~ — Register / Yank

 | Key     | Command                 | Description     |
 |---------+-------------------------+-----------------|
 | ~r r~   | ~consult-register~      | Registers       |
 | ~r s~   | ~consult-register-store~| Store register  |
 | ~r y~   | ~consult-yank-pop~      | Yank pop        |
 | ~r m~   | ~consult-kmacro~        | Kmacro          |
...
***** ~SPC t~ — Toggle / Tools
 ...
 |---------+------------------------------------------------+----------------------|
 | ~t d~   | ~docker~                                       | Docker               |
 | ~t w~   | ~whisper-run~                                  | Whisper (speech)     |
 | ~t f~   | ~treesit-fold-toggle~                          | Fold toggle          |
 | ~t p~   | ~flymake-show-project-diagnostics~             | Project diagnostics  |
 | ~t P~   | ~toggle-flymake-show-diagnostics-at-end-of-line~ | Inline diagnostics |
 | ~t s~   | ~gptel-toggle-sidebar~                         | AI sidebar           |
 | ~t e~   | ~er/expand-region~                             | Expand region        |
 | ~t m~   | ~lasgun-transient~                             | Multi-cursor         |

***** ~SPC x~ — Execute

 | Key     | Command               | Description     |
 |---------+-----------------------+-----------------|
 | ~x x~   | ~consult-taskfile~    | Taskfile        |
 | ~x c~   | ~taskfile~            | Taskfile menu   |
 | ~x m~   | ~consult-mode-command~| Mode command    |
 | ~x M~   | ~consult-man~         | Man pages       |
 | ~x i~   | ~consult-info~        | Info            |
 | ~x h~   | ~consult-history~     | History         |
 | ~x e~   | ~gptel-menu~          | AI/gptel menu   |

***** ~SPC c~ — Code / LSP

 | Key     | Command                              | Description       |
 |---------+--------------------------------------+-------------------|
 | ~c f~   | ~consult-eglot-symbols~              | Symbols           |
 | ~c r~   | ~eglot-rename~                       | Rename symbol     |
 | ~c a~   | ~eglot-code-actions~                 | Code actions      |
 | ~c q~   | ~eglot-code-action-quickfix~         | Quickfix          |
 | ~c i~   | ~eglot-find-implementation~          | Find impl         |
 | ~c t~   | ~eglot-find-typeDefinition~          | Find type def     |
 | ~c o~   | ~eglot-code-action-organize-imports~ | Organize imports  |
...
** Spelling
   ...
   zUsing  =flyspell-correct= as it uses =completing-read= by default so that it uses consult for it's completion
...
Agenda, clock, and type-break state consumed by the Caelestia Tasks pane.
...
*** Mise
   ...
   [[https://github.com/eki3z/mise.el][mise.el]] integrates the mise tool into Emacs to manage per-directory environment variables on a per-buffer basis. Automatically detects and applies project-specific configurations.
...
Records from the default PulseAudio source via FFmpeg, pipes the WAV
to ~whisper-cpp~ (Vulkan-accelerated on redfish), inserts the
transcription at point.  Model ships from Guix Home as part of
~%ai-packages~.  Intended workflow: open a gptel buffer, press
~SPC t v~, speak, press it again to stop; transcript lands in the
