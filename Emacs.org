...
- [[#program-settings][Program Settings]]
  ...
  - [[#change-emacs-directory][Change Emacs Directory]]
...
** Startup Performance
   ...
   Make startup faster by reducing the frequency of garbage collection and then use a hook to measure Emacs startup time. Startup time is bad because I have stopped caring about startup time after switching to running Emacs as a daemon.
...
** Update Load Path
   ...
   Custom Emacs Lisp libraries which must be added to the load path. Currently only =init-windows.el= needs to be loaded.
...
** Startup Message
   ...
   The default scratch buffer message.
...
I don't know if this is necessary in Emacs 28 but I'm keeping it.
...
** Calendar
   ...
   calfw Calendar renders a nice looking calendar in what looks to be a org table. Displays holidays,  org agenda and ical backups.
...
#+begin_src emacs-lisp
  ...
  ;; (ad
  ;; d-hook 'cc-mode-hook #'lsp-cpp-install-save-hooks)
#+end_src
